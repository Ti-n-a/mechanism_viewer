...
- `rows_with_similar_missing`: UpSet similar plot counting the row-level missing combinations.


## Missingness Compared to Complete Columns 

- `build_distribution_of_missingness`: plot distribution of an observed column based on the missingness of a column.
...
- `complete_and_missing_columns_correlation`: correlation between the values of complete columns and the missingness indicator of missing columns.
- `missing_vs_all_correlation`: correlation between the missingness indicator of a missing column and the values of all other columns.


## Statistical Tests and MAR Heuristic

- `little_mcar_test` and `interpret_mcar_p_value`: global Little's MCAR test via `pyampute` and its text interpretation.
...
- `scatterplot_imputation_comparison`: compare observed and imputed values against an observed column.
...
- `little_mcar_test` and `little_mcar_pairwise` require numeric columns and sufficient missing/non-missing counts.
